...
- 内置与外部 ADB 都作为无控制台窗口的后台子进程运行，不会在刷新、截图、点击或预览时弹出黑色窗口。
- 验证并选择同一个 `adb.exe` 管理服务器、设备、截图与点击。
- 自动发现并连接本机正在运行、ADB 已启用的 MuMu 12 实例；展示在线、离线及未授权设备，仅允许选择在线设备。
- 通过 IP 地址和端口请求 `adb connect`。
- 手动获取 PNG 截图，在缩放或留白后的画面中准确换算坐标。
- 单击选择坐标或四向拖拽框选区域；显式执行后由 Rust 在合法范围内随机延时、落点并模拟随机按压，等待期间可以取消。
- 在本机校准结界突破的八类页面、九个对手区域、刷新按钮和已知弹窗；仅保存不可还原视觉特征与坐标，不保存或分发原始截图、裁剪模板或游戏素材。
- 运行单活动设备的可暂停结界突破循环；目标为 1–30 次成功结算，胜利返回九宫格后计数，失败不计数并切换下一对手。
...
3. 应用启动和刷新设备列表时，会自动连接本机正在运行且 ADB 端口可归因于 MuMu 进程的 MuMu 12 实例。只有一台在线设备时会自动选择，多台设备时保留仍在线的当前选择。
4. 未运行、尚未启用 ADB 或无法自动识别的设备不会被启动；如设备未出现，可填写模拟器提供的 IP 和端口进行手动连接。单个自动连接失败不会隐藏其他已经可用的设备。
5. 点击“刷新截图”，单击取点、拖拽框选区域或用键盘编辑 X/Y。
6. 检查目标和随机点击配置后点击“执行点击”；随机延时期间可取消。切换设备会清除旧截图和目标。
7. 自动挑战前，在“结界突破校准”中依次校准庭院探索入口、探索地图入口、九宫格、对手详情、战斗准备、战斗中、胜利奖励和失败结算；为九宫格按顺序框选九个对手区域并分别添加“可挑战”特征，再配置刷新按钮及“刷新可用”特征、3/6/9 奖励及“可领取”特征。在对手详情页至少添加目标详情与“消耗 1”两项进攻门禁。
8. 可按需配置“挑战券不足”等安全暂停条件和 1–9 的连续失败上限。保存配置后设置 1–30 次目标成功次数并开始任务。程序只尝试通过可挑战门禁的未尝试对手；没有可信目标且刷新可用时才刷新。胜利结算后计数，失败不计数。暂停后先“重新识别”，确认页面结果后再恢复；关闭应用不会恢复任务或继续点击。

也可点击“实时预览（原型）”验证连续画面。此路径临时使用 Android `screenrecord` 输出的 H.264 Annex-B 流和 WebView2 WebCodecs，默认录制时长及编码能力受设备实现限制；停止后可立即重新开始。连续 8 秒没有码流数据、读取失败、自然结束或进程异常退出会分别显示诊断信息，并回到“刷新截图”。Windows 实测方法与结果表见 [Issue #4 实时预览原型](docs/prototypes/issue-4-live-preview.md)。
...
- MuMu 自动发现通过 Windows TCP 监听表取得端口及所属 PID，只连接本机 MuMu/Nemu 可执行文件持有的端口，并在连接后读取 Android 设备型号；不调用 shell、`netstat` 或进行全端口扫描。
...
获取截图后，选择“创建区域”，在画面内向任意方向拖拽框选；选区实时显示原始像素尺寸。拖动区域内部或中心十字手柄移动，拖动八个边角手柄调整大小。小选区的编辑手柄在外围分开显示，不改变设备像素区域。指针离开画面后释放也会结束编辑，区域始终限制在画面内且宽高至少 1 像素。

Left / Top / Width / Height 与鼠标编辑同步，可用于精调。从当前点创建的 40 × 40 区域只是初始大小。“选择单点”可清除矩形并返回取点模式。框选、移动和缩放只编辑目标；手动操作设备仍需按“执行点击”。区域可用于随机点击、识别特征、动作区域及弹窗校准。刷新截图、停止预览或切换设备会清除选区和拖拽状态。
