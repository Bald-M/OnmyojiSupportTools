...
- 自动发现并连接本机正在运行、ADB 已启用的 MuMu 12 实例；活动设备列表优先显示 MuMu 多开器中的实例名称，并只保留在线可操作设备，避免已关闭实例留下的离线地址干扰选择。
...
3. 应用启动和刷新设备列表时，会自动连接本机正在运行且 ADB 端口可归因于 MuMu 进程的 MuMu 12 实例，并通过安装目录中的 `MuMuManager.exe info -v all` 取得实例名称与实际 ADB 端口。管理工具不可用时显示包含 ADB endpoint 的唯一回退名称。只有一台在线设备时会自动选择，多台设备时保留仍在线的当前选择，离线及未授权条目不进入活动设备列表。
...
- MuMu 自动发现通过 Windows TCP 监听表取得端口及所属 PID，从已验证的 MuMu 进程安装目录定位官方 `MuMuManager.exe`，以结构化参数和超时执行只读的 `info -v all` 查询实例名称与端口；只连接本机 MuMu/Nemu 可执行文件持有的端口，并在连接后读取 Android 设备型号，不调用 shell、`netstat` 或进行全端口扫描。
