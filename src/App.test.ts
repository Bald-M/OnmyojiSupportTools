...
describe('desktop device workflow', () => {
  ...
  it('normalizes a dragged region and submits it as the click target', async () => {
    ...
    await pointer(stage.element, 'pointerdown', 750, 500)
    await stage.trigger('mousedown', { clientX: 750, clientY: 500 })
    await pointer(stage.element, 'pointermove', 250, 250)
    expect(wrapper.get('[data-testid="selection-size"]').text()).toContain('500 × 250')
    await pointer(stage.element, 'pointerup', 250, 250)
    await stage.trigger('mouseup', { clientX: 250, clientY: 250 })
    await stage.trigger('click', { clientX: 250, clientY: 250 })
  ...
  it('creates, moves and resizes a scaled region without device input, and synchronizes numeric edits', async () => {
    const wrapper = mount(App)
    await flushPromises()
    await wrapper.get('[data-testid="capture-button"]').trigger('click')
    await flushPromises()
    Object.defineProperties(wrapper.get('img').element, { naturalWidth: { value: 1000 }, naturalHeight: { value: 500 } })
    await wrapper.get('img').trigger('load')
    const stage = wrapper.get('[data-testid="screenshot-stage"]')
    vi.spyOn(stage.element, 'getBoundingClientRect').mockReturnValue({ left: 0, top: 0, width: 500, height: 500, right: 500, bottom: 500, x: 0, y: 0, toJSON: () => ({}) })
    const capture = vi.fn()
    const release = vi.fn()
    Object.assign(stage.element, { setPointerCapture: capture, hasPointerCapture: () => true, releasePointerCapture: release })
    await wrapper.findAll('button').find(button => button.text() === '创建区域')!.trigger('click')
    await pointer(stage.element, 'pointerdown', 50, 50)
    await pointer(stage.element, 'pointerup', 100, 100)
    expect(wrapper.find('[data-testid="selection-size"]').exists()).toBe(false)
    await pointer(stage.element, 'pointerdown', 200, 300)
    await pointer(stage.element, 'pointermove', 50, 150)
    await pointer(stage.element, 'pointerup', 50, 150)
    await stage.trigger('click', { clientX: 50, clientY: 150 })
    const fields = () => wrapper.findAll('[aria-label="点击区域像素边界"] input')
    const values = () => fields().map(field => (field.element as HTMLInputElement).value)
    expect(values()).toEqual(['100', '50', '300', '300'])
    await pointer(wrapper.get('.selection-rect').element, 'pointerdown', 100, 200)
    await pointer(stage.element, 'pointermove', 900, 900)
    await pointer(stage.element, 'pointerup', 900, 900)
    expect(values()).toEqual(['700', '200', '300', '300'])
    expect(capture).toHaveBeenCalledWith(1)
    expect(release).toHaveBeenCalledWith(1)
    await pointer(stage.element, 'pointermove', 10, 130)
    expect(values()).toEqual(['700', '200', '300', '300'])
    await pointer(wrapper.get('[data-resize-handle="nw"]').element, 'pointerdown', 350, 225)
    await pointer(stage.element, 'pointerup', 900, 900)
    expect(values()).toEqual(['999', '499', '1', '1'])
    await fields()[0]!.setValue('20')
    await fields()[1]!.setValue('30')
    await fields()[2]!.setValue('100')
    await fields()[3]!.setValue('80')
    expect(wrapper.get('.selection-rect').attributes('style')).toContain('width: 50px')
    await pointer(wrapper.get('[data-resize-handle="se"]').element, 'pointerdown', 60, 180)
    await pointer(stage.element, 'pointerup', 100, 200)
    expect(values()).toEqual(['20', '30', '180', '120'])
    expect(device.tapScreen).not.toHaveBeenCalled()
    await wrapper.get('[data-testid="capture-button"]').trigger('click')
    await flushPromises()
  ...
  it.each(['n', 'ne', 'e', 'se', 's', 'sw', 'w', 'nw'])('resizes the %s edge with legal boundaries', async (handle) => {
    const wrapper = mount(App)
    await flushPromises()
    await wrapper.get('[data-testid="preview-button"]').trigger('click')
    await flushPromises()
    const stage = wrapper.get('[data-testid="screenshot-stage"]')
    vi.spyOn(stage.element, 'getBoundingClientRect').mockReturnValue({ left: 0, top: 0, width: 1280, height: 720, right: 1280, bottom: 720, x: 0, y: 0, toJSON: () => ({}) })
    await pointer(stage.element, 'pointerdown', 100, 100)
    await pointer(stage.element, 'pointerup', 200, 200)
    await pointer(wrapper.get(`[data-resize-handle="${handle}"]`).element, 'pointerdown', 150, 150)
    await pointer(stage.element, 'pointerup', handle.includes('w') ? 0 : 250, handle.includes('n') ? 0 : 250)
    const values = wrapper.findAll('[aria-label="点击区域像素边界"] input').map(field => Number((field.element as HTMLInputElement).value))
    expect(values).toEqual([handle.includes('w') ? 0 : 100, handle.includes('n') ? 0 : 100, handle.includes('w') || handle.includes('e') ? 200 : 100, handle.includes('n') || handle.includes('s') ? 200 : 100])
    expect(device.tapScreen).not.toHaveBeenCalled()
    await stage.trigger('pointercancel')
    wrapper.unmount()
  })

  it('clears a captured edit on device changes and allows a fresh selection after cancellation', async () => {
    const wrapper = mount(App)
    await flushPromises()
    await wrapper.get('[data-testid="preview-button"]').trigger('click')
    await flushPromises()
    ...
    vi.spyOn(stage.element, 'getBoundingClientRect').mockReturnValue({ left: 0, top: 0, width: 1280, height: 720, right: 1280, bottom: 720, x: 0, y: 0, toJSON: () => ({}) })
    await pointer(stage.element, 'pointerdown', 100, 100)
    await pointer(stage.element, 'pointermove', 200, 200)
    await stage.trigger('pointercancel')
    await pointer(stage.element, 'pointermove', 400, 400)
    expect(wrapper.get('[data-testid="selection-size"]').text()).toContain('100 × 100')
    await pointer(stage.element, 'pointerdown', 300, 300)
    await pointer(stage.element, 'pointermove', 500, 500)
    device.refreshDevices.mockResolvedValueOnce({ ...readyState, activeDeviceSerial: null, devices: [] })
    await wrapper.get('[aria-label="刷新设备列表"]').trigger('click')
    await flushPromises()
    await pointer(stage.element, 'pointerup', 600, 600)
    expect(wrapper.find('.selection-rect').exists()).toBe(false)
    expect(device.tapScreen).not.toHaveBeenCalled()
    wrapper.unmount()
  })

  it('keeps tiny rectangles legal, clamps numeric input immediately and returns explicitly to point selection', async () => {
    const wrapper = mount(App)
    await flushPromises()
    await wrapper.get('[data-testid="preview-button"]').trigger('click')
    await flushPromises()
    ...
    vi.spyOn(stage.element, 'getBoundingClientRect').mockReturnValue({ left: 0, top: 0, width: 1280, height: 720, right: 1280, bottom: 720, x: 0, y: 0, toJSON: () => ({}) })
    await wrapper.findAll('button').find(button => button.text() === '创建区域')!.trigger('click')
    await pointer(stage.element, 'pointerdown', 1280, 720)
    await pointer(stage.element, 'pointerup', 1280, 720)
    await stage.trigger('click', { clientX: 1280, clientY: 720 })
    const fields = wrapper.findAll('[aria-label="点击区域像素边界"] input')
    const values = () => fields.map(field => (field.element as HTMLInputElement).value)
    expect(values()).toEqual(['1279', '719', '1', '1'])
    expect(wrapper.get('#coordinate-x').attributes('disabled')).toBeDefined()
    await fields[0]!.setValue('-10')
    await fields[1]!.setValue('3.8')
    await fields[2]!.setValue('9999')
    await fields[3]!.setValue('0')
    expect(values()).toEqual(['0', '3', '1280', '1'])
    await wrapper.findAll('button').find(button => button.text() === '选择单点')!.trigger('click')
    await pointer(stage.element, 'pointerdown', 30, 40)
    await pointer(stage.element, 'pointerup', 30, 40)
    await stage.trigger('click', { clientX: 30, clientY: 40 })
    expect(wrapper.find('.selection-rect').exists()).toBe(false)
    expect((wrapper.get('#coordinate-x').element as HTMLInputElement).value).toBe('30')
    expect((wrapper.get('#coordinate-y').element as HTMLInputElement).value).toBe('40')
    expect(device.tapScreen).not.toHaveBeenCalled()
    wrapper.unmount()
  })
  ...
  it('starts and stops preview explicitly without tapping the device', async () => {
    ...
    await pointer(stage.element, 'pointerdown', 100, 100)
    await pointer(stage.element, 'pointermove', 200, 200)
    expect(wrapper.find('.selection-rect').exists()).toBe(true)
    await wrapper.get('[data-testid="preview-button"]').trigger('click')
    await flushPromises()
    expect(device.stopPreview).toHaveBeenCalledOnce()
    expect(wrapper.find('.selection-rect').exists()).toBe(false)
    expect((wrapper.get('#coordinate-x').element as HTMLInputElement).value).toBe('')
    await pointer(stage.element, 'pointermove', 100, 100)
    expect(wrapper.find('.selection-rect').exists()).toBe(false)
    wrapper.unmount()
  })
  ...
  it('calibrates recognition and safe actions for every required page state', async () => {
    ...
    await pointer(stage.element, 'pointerdown', 100, 50)
    ...
    await flushPromises()
    await wrapper.findAll('button').find(button => button.text() === '设为弹窗关闭区域')!.trigger('click')
