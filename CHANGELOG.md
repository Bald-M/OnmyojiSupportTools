...
- 新增本机 MuMu 12 运行实例自动发现：通过 Windows 监听端口及进程归属识别候选 ADB 端口，逐个连接并验证 Android 设备属性；单个实例失败不会影响其他设备，手动连接入口继续保留。

- 支持在截图和实时预览中鼠标框选、移动及八方向缩放点击与校准区域，实时显示像素尺寸并与数字边界同步；画面外释放正确结束编辑，拖拽后的 click 不覆盖矩形。
...
- 新增结界突破专用八状态流程、九宫格对手与刷新区域校准、1–30 次成功目标，以及胜利计数/失败不计数分支。
...
- 修复 Windows 上刷新设备、连接、截图、点击和实时预览等 ADB 操作会弹出控制台窗口的问题；内置与外部 ADB 现在都通过统一的无窗口进程入口启动。
- 修复实时预览结束原因不可区分的问题；持续码流会刷新 8 秒 watchdog，停滞、EOF、读取失败和非零退出现在分别报告诊断信息。
